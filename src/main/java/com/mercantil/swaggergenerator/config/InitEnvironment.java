 ...
 *         <hr>
 *         <code>Copyright 2026. Banco Mercantil, All rights reserved.</code>
 */
public class InitEnvironment {
	...
	/**
	 * METODO ENCARGADO DE OBTENER LA CONFIGURACION DEL GENERADOR.
	 */
	public static void getConfig() {
		...
		try (InputStream is = new FileInputStream(pathFile)) {
			...
			pathFileRules = Utils.resolvePath(props.getProperty(BusinessCard.RULES.getValue()));

			pathFileAbbreviations = Utils.resolvePath(props.getProperty(BusinessCard.ABBREVIATIONS.getValue()));

			pathServices = Utils.resolvePath(props.getProperty("pathServices"));

			// =================================================
			// ✅ REGISTRAR EN SYSTEM (🔥 FALTABA ESTO)
			// =================================================
			System.setProperty("pathServices", pathServices);
			System.setProperty("pathRules", pathFileRules);
			System.setProperty("pathAbbreviations", pathFileAbbreviations);
			System.setProperty("pathOutput", Utils.resolvePath(props.getProperty("pathOutput")));

			// =================================================
			// ✅ VALIDAMOS LOS ARCHIVOS DE CONFIGURACION
			// =================================================
			FileUtility.validatePaths(pathFile, pathFileLog4j2, pathFileRules, pathFileAbbreviations, pathServices);
